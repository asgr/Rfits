...
// [[Rcpp::export]]
SEXP Cfits_read_img_omp(Rcpp::String filename, int ext=1, int datatype= -32,
                    long naxis1=100, long naxis2=100, long naxis3=1, long naxis4=1, int nthreads=2)
{
  int anynull, nullvals = 0, hdutype, extra;
  std::vector<int> offset(nthreads);
  ...
  int elements_per_thread = nelements / nthreads;
  int remainder = nelements % nthreads;
  for (int i = 0; i != nthreads; i++) {
    extra = (i < remainder) ? 1 : 0;
    offset[i] = elements_per_thread * i + extra;
    count[i] = elements_per_thread + extra;
  }
...
#pragma omp parallel for schedule(static) num_threads(nthreads)
    for (int i = 0; i != nthreads; i++) {
      fits_invoke(read_img, fptr, TFLOAT, offset[i], count[i], nullptr, pixels.data() + offset[i], &anynull);
    }
    Rcpp::NumericVector pixel_matrix(naxis1 * naxis2 * naxis3 * naxis4);
    std::copy(pixels.begin(), pixels.end(), pixel_matrix.begin());
    return(pixel_matrix);
...
#pragma omp parallel for schedule(static) num_threads(nthreads)
    for (int i = 0; i != nthreads; i++) {
      fits_invoke(read_img, fptr, TDOUBLE, offset[i], count[i], nullptr, pixels.data() + offset[i], &anynull);
    }
    Rcpp::NumericVector pixel_matrix(naxis1 * naxis2 * naxis3 * naxis4);
    std::copy(pixels.begin(), pixels.end(), pixel_matrix.begin());
    return(pixel_matrix);
...
#pragma omp parallel for schedule(static) num_threads(nthreads)
    for (int i = 0; i != nthreads; i++) {
      fits_invoke(read_img, fptr, TBYTE, offset[i], count[i], nullptr, pixels.data() + offset[i], &anynull);
    }
    Rcpp::IntegerVector pixel_matrix(naxis1 * naxis2 * naxis3 * naxis4);
    std::copy(pixels.begin(), pixels.end(), pixel_matrix.begin());
    return(pixel_matrix);
...
#pragma omp parallel for schedule(static) num_threads(nthreads)
    for (int i = 0; i != nthreads; i++) {
      fits_invoke(read_img, fptr, TLONG, offset[i], count[i], nullptr, pixels.data() + offset[i], &anynull);
    }
    Rcpp::IntegerVector pixel_matrix(naxis1 * naxis2 * naxis3 * naxis4);
    std::copy(pixels.begin(), pixels.end(), pixel_matrix.begin());
    return(pixel_matrix);
...
#pragma omp parallel for schedule(static) num_threads(nthreads)
    for (int i = 0; i != nthreads; i++) {
      fits_invoke(read_img, fptr, TLONGLONG, offset[i], count[i], nullptr, pixels.data() + offset[i], &anynull);
    }
    Rcpp::NumericVector pixel_matrix(naxis1 * naxis2 * naxis3 * naxis4);
    std::memcpy(&(pixel_matrix[0]), &(pixels[0]), nelements * sizeof(double));
    pixel_matrix.attr("class") = "integer64";
