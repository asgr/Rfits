...
SEXP Cfits_read_col(Rcpp::String filename, int colref=1, int ext=2, 
  ...
  if (nrow == 1) {
    ...
    fits_read_key(fptr, TLONG, const_cast<char *>("NELEM"), &nelem, comment, &status_key);
    
    if (status_key == 0 && nelem > 0) {
      Rcpp::warning("NAXIS2=1; column likely stores vector with repeat=" + std::to_string(repeat));
    }
  }
